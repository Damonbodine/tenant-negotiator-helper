...
export const Header = () => {
  ...
  const navLinks = [
    { name: "Price Analysis", path: "/", icon: <Activity className="h-5 w-5 mr-1" />, action: () => window.location.href = "/?journey=market" },
    { name: "Compare", path: "/", icon: <FileSearch className="h-5 w-5 mr-1" />, action: () => window.location.href = "/?journey=comparison" },
    { name: "Get Help", path: "/", icon: <HelpCircle className="h-5 w-5 mr-1" />, action: () => window.location.href = "/?journey=negotiation" },
    { name: "Practice", path: "/practice/voice", icon: <Play className="h-5 w-5 mr-1" /> },
    { name: "Lease Review", path: "/lease-analyzer", icon: <FileText className="h-5 w-5 mr-1" /> },
    { name: "Scripts", path: "/script-builder", icon: <Code className="h-5 w-5 mr-1" /> },
    { name: "Resources", path: "/resources", icon: <Book className="h-5 w-5 mr-1" /> }
  ];
                  ...
                  {item.action ? (
                    <button 
                      ...
                      className={cn(
                        navigationMenuTriggerStyle(),
                        "flex items-center",
                        (location.search.includes(`journey=${item.path.replace('/?journey=', '')}`) || 
                         (item.path === '/' && !location.search && location.pathname === '/'))
                          ? "bg-cyan-400/10 text-cyan-400" 
                          : "text-cyan-400/80 hover:text-cyan-400 hover:bg-cyan-400/10"
                      )}
                    >
                      {item.icon}
                      {item.name}
            ...
            <div key={item.name}>
              {item.action ? (
                <button
                  onClick={item.action}
                  className={cn(
                    "flex flex-col items-center px-1 py-1 rounded",
                    (location.search.includes(`journey=${item.path.replace('/?journey=', '')}`) || 
                     (item.path === '/' && !location.search && location.pathname === '/'))
                      ? "text-cyan-400" 
                      : "text-cyan-400/70"
